...
predictor<-xreturns$EPS

# Create a new data frame of predictor, variables used to construct predictor, and current returns 
sortdat<-data.table(permno=xreturns$permno,date=xreturns$date,eret=xreturns$eret,predictor, act=xreturns$act, lct=xreturns$lct)
...
sumstat<-data.table(predictor=sortdat$predictor,eret=sortdat$eret, lct=sortdat$lct, act=sortdat$act)
sink("summary.txt")
...
HP<-decileports_wide[10,]-decileports_wide[1,] # returns of the long-short decile hedge portfolio over time
